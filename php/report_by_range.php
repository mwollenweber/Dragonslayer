...
if(is_null($start) || is_null($end))
  {
    die("please provide a start and end date");
  }
...
$weekly_ips_query = sprintf("SELECT INET_NTOA(victim), network, discovered, notes FROM gwcases WHERE (report_category >= 100) AND DATE(tdstamp) BETWEEN '%s' and '%s' GROUP BY victim ORDER BY tdstamp, victim", $start, $end);
...
for($x = 0; $x < $ip_count; $x++)
  {
    ...
    $patchy_results = mysql_query($patchy_query . $results_table[$x][1] . "'");
