...
$weekly_ticket_query = 
  "SELECT id, tdstamp, reporter, report_category, event, INET_NTOA(victim), INET_NTOA(attacker), dns_name, network, verification FROM gwcases WHERE DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), DAYOFWEEK(CURDATE())) and CURDATE() AND report_category > 0 ORDER BY tdstamp";

$weekly_vip_query = 
  "SELECT id, tdstamp, reporter, report_category, event, INET_NTOA(victim), INET_NTOA(attacker), dns_name, network, verification FROM gwcases WHERE DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), DAYOFWEEK(CURDATE())) and CURDATE() AND report_category > 201 ORDER BY tdstamp";
   
if(!is_null($category))
  {
    if($category == "vip" || $category == "VIP")
      {
	$results = mysql_query($weekly_vip_query); 
      }
    //elseif(bsdfs)
  }
...
<?
$total_cases_query = "SELECT COUNT(*) FROM gwcases WHERE DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), DAYOFWEEK(CURDATE())) and CURDATE()  AND report_category > 1";
...
$normal_case_query = "SELECT COUNT(*) FROM gwcases WHERE DATE(tdstamp) BETWEEN  SUBDATE(CURDATE(), DAYOFWEEK(CURDATE())) and CURDATE() AND report_category >= 100";
