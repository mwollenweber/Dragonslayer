...
if ($type == 'week' || $type == 'month' || $type == 'year') {
	...
	$data = array();
	$data = array(array("All", "Student", "DIT"));
	
	$instance = array();
	
	#Cases per week
	$query = "SELECT COUNT(id) as c, week(tdstamp) as w from gwcases where DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), 180) AND CURDATE() group by week(tdstamp) ORDER BY id, week(tdstamp)";
	$result= mysqli_query($link,$query);
	while($row = mysqli_fetch_assoc($result)) {
		$point = array((int)$row['w'],(int)$row['c']);
		$instance[] = $point;
	}
	...
	#Student cases
	$query = "select count(id) as c, week(tdstamp) as w from gwcases where DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), 180) AND CURDATE() AND report_category = 20 group by week(tdstamp) ORDER BY id, week(tdstamp)";
	$result= mysqli_query($link,$query);
	while($row = mysqli_fetch_assoc($result)) {
		$point = array((int)$row['w'],(int)$row['c']);
		$instance[] = $point;
	...
	#DIT cases
	$query = "select count(id) as c, week(tdstamp) as w from gwcases where DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), 180) AND CURDATE() AND network like '%DIT%'  and  network not like '%guest%' GROUP BY week(tdstamp) ORDER BY id, week(tdstamp)";
	$result= mysqli_query($link,$query);
	while($row = mysqli_fetch_assoc($result)) {
		$point = array((int)$row['w'],(int)$row['c']);
		$instance[] = $point;
