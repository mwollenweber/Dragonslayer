...
#Reporter Query
$reporters = array();
$query= "SELECT distinct(reporter) from gwcases WHERE YEAR(tdstamp) = YEAR(CURRENT_TIMESTAMP) AND reporter <> \"\" AND DATE(tdstamp) BETWEEN SUBDATE(CURDATE(), 60) AND CURDATE() AND YEARWEEK(tdstamp) = YEARWEEK(CURRENT_TIMESTAMP)";
